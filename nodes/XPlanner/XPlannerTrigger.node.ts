...
export class XPlannerTrigger implements INodeType {
    description: INodeTypeDescription = {
        displayName: 'XPlanner Trigger',
        name: 'xplannerTrigger',
        icon: { light: 'file:xplanner.svg', dark: 'file:xplanner.svg' },
        group: ['trigger'],
        version: 1,
        description: 'Start de workflow wanneer er iets gebeurt in XPlanner',
        defaults: {
            name: 'XPlanner Trigger',
        },
        inputs: [],
    ...
    methods = {
        loadOptions: {
            async getModels(this: ILoadOptionsFunctions): Promise<INodePropertyOptions[]> {
                ...
                const baseUrl = credentials.baseUrl as string;
                const accessToken = await XPlannerTrigger.getAccessToken(this, credentials);
