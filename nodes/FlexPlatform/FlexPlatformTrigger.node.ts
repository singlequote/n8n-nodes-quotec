...
export class FlexPlatformTrigger implements INodeType {
    description: INodeTypeDescription = {
        displayName: 'Flex Platform Trigger',
        name: 'flexPlatformTrigger',
        icon: { light: 'file:flex-platform.svg', dark: 'file:flex-platform.svg' },
        group: ['trigger'],
        version: 1,
        description: 'Start de workflow wanneer er iets gebeurt in Flex Platform',
        defaults: {
            name: 'Flex Platform Trigger',
        },
        inputs: [],
        ...
        properties: [
            {
                ...
                type: 'options',
                default: '',
                description: 'Filter events op module',
                typeOptions: {
    ...
    methods = {
        loadOptions: {
            async getModules(this: ILoadOptionsFunctions): Promise<INodePropertyOptions[]> {
                ...
                const baseUrl = credentials.baseUrl as string;
                const accessToken = await FlexPlatformTrigger.getAccessToken(this, credentials);
                ...
                for (const event of response.data) {
                    if (event.module && !uniqueModules.has(event.module)) {
                        uniqueModules.add(event.module);
                        returnOptions.push({
                            name: event.module.charAt(0).toUpperCase() + event.module.slice(1),
                            value: event.module,
                        });
                    }
                }
                ...
                returnOptions.unshift({ name: '- Alle Modules -', value: '' });
            ...
            async getEvents(this: ILoadOptionsFunctions): Promise<INodePropertyOptions[]> {
                ...
                const selectedModule = this.getNodeParameter('module') as string;
                ...
                if (selectedModule) {
                    options.qs['module'] = selectedModule;
                }
            ...
            async delete(this: IHookFunctions): Promise<boolean> {
                ...
                if (webhookData.webhookId) {
                    ...
                    try {
                        ...
                        // @ts-ignore
                        console.error('Kon webhook niet verwijderen', error);
                    }
